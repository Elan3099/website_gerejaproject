...
<div class="container" style="margin-top: 50px;">
	...
	<div class="textup" style="font-family: 'Ramabhadra', sans-serif; color: white;">
		<b><p>DATA REGRISTRASI JEMAAT IBADAH RAYA I</p></b>
		...
		<div class="row">
			...
			<form action="/cari" method="GET" style="margin-bottom: 20px;">
				<input type="text" name="cari" placeholder="Cari Kode .." value="{{ old('cari') }}">
			...
			<table class="table " style="margin-bottom: 100px; background-color: #eeeeee; text-align: center;">
				...
				<tbody>
					...
					<tr>
						...
						<td td data-header="Gender"> {{ $d -> gender }}</td>
						<td td data-header="Alamat">{{ $d -> alamat }}</td>
						<td td data-header="Email">{{ $d -> email }}</td>
						<td td data-header="Tanggal">{{ $d -> tgl }}</td>
						<td td data-header="Waktu">{{ $d -> waktu }}</td>
						<th scope="row">
							<div class="toolbox">
								<a href="absensi/{{ $d->id }}" type="button"  class="btn btn-primary" >HADIR</a>
							</div>
						</th>
